...
class Settings(BaseSettings):
    ...
    bot_poll_interval_seconds: float = Field(default=1.0, alias="BOT_POLL_INTERVAL_SECONDS")

    scheduler_enabled: bool = Field(default=True, alias="SCHEDULER_ENABLED")
    scheduler_interval_hours: int = Field(default=6, alias="SCHEDULER_INTERVAL_HOURS")

    max_jobs_per_query: int = Field(default=20, alias="MAX_JOBS_PER_QUERY")
    scraping_concurrency: int = Field(default=8, alias="SCRAPING_CONCURRENCY")
    request_timeout_seconds: int = Field(default=20, alias="REQUEST_TIMEOUT_SECONDS")
    min_delay_seconds: float = Field(default=1.0, alias="MIN_DELAY_SECONDS")
    max_delay_seconds: float = Field(default=3.5, alias="MAX_DELAY_SECONDS")
